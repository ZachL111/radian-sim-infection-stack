...
`radian-sim-infection-stack` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for infection workflows, centered on incremental indexing, append-only fixtures, and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 164. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 4.
